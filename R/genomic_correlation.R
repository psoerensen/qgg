...
ldsc <- function(Glist=NULL, ldscores=NULL, sets=NULL, method="regression", residual=FALSE, z=NULL, b=NULL, seb=NULL, af=NULL, stat=NULL, tol=1e-8,
  ...
  for ( t in 1:nt) {
    ...
    if(intercept) X <- cbind(1,n[names(z2),t]*ldscores[names(z2)]/length(z2))
    if(!intercept) X <- matrix(n[names(z2),t]*ldscores[names(z2)]/length(z2),ncol=1)
    y <- z2
    XtX <- crossprod(X)
    Xy <- crossprod(X,y)
  ...
  if(SE.h2==TRUE){
    if(intercept==TRUE){
      ...
      for ( t in 1:nt) {
        ...
        z2 <- z2[!is.na(z2)]
        z2 <- z2[z2<maxZ2]
        X <- cbind(1,n[names(z2),t]*ldscores[names(z2)]/length(z2))
        y <- z2
        XtX <- crossprod(X)
        Xy <- crossprod(X,y)
    ...
    if(intercept==FALSE){
      ...
      for ( t in 1:nt) {
        ...
        z2 <- z2[!is.na(z2)]
        z2 <- z2[z2<maxZ2]
        X <- matrix(n[names(z2),t]*ldscores[names(z2)]/length(z2),ncol=1)
        y <- z2
        XtX <- crossprod(X)
        Xy <- crossprod(X,y)
  ...
  if(what=="rg") {
    ...
    for (t1 in 1:nt) {
      for (t2 in t1:nt) {
        ...
        rws <- rws1 & rws2
        m <- sum(rws)
        X <- cbind(1,sqrt(n[rws,t1])*sqrt(n[rws,t2])*ldscores[rws]/m)
        y <- Z1[rws]*Z2[rws]
        XtX <- crossprod(X)
        Xy <- crossprod(X,y)
        ...
        for (t1 in 1:nt) {
          for (t2 in t1:nt) {
            if(t1!=t2){
              ...
              X1 <- cbind(1,n[rws,t1]*ldscores[rws]/sum(rws))
              y1 <- Z2_1[rws]
              X2 <- cbind(1,n[rws,t2]*ldscores[rws]/sum(rws))
              y2 <- Z2_2[rws]
        ...
        for (t1 in 1:nt) {
          for (t2 in t1:nt) {
            if(t1!=t2){
              ...
              X1 <- matrix(n[rws,t1]*ldscores[rws]/sum(rws),ncol=1)
              y1 <- Z2_1[rws]
              X2 <- matrix(n[rws,t2]*ldscores[rws]/sum(rws),ncol=1)
              y2 <- Z2_2[rws]
              ...
              X <- matrix(1,sqrt(n[rws,t1])*sqrt(n[rws,t2])*ldscores[rws]/m,ncol=1)
              y <- Z1[rws]*Z2[rws]
              XtX <- crossprod(X)
              Xy <- crossprod(X,y)
