...
gfm <- function(fm = NULL, weights = NULL, W = NULL, sets = NULL, G = NULL, data = NULL, validate = NULL, mkplots = FALSE) {
  ...
  # Validation sets
  if (is.null(validate)) {
    validate <- matrix(-c(1:n), nrow = n, ncol = 1)
  }
  ...
  # Check input data
  if (!is.null(sets)) {
    if (any(!sapply(sets, is.character))) {
      stop("Sets not character variables")
    }
  }
  if (!is.null(W)) {
    if (any(!is.character(colnames(W)))) {
      stop("Column names of W is not character variables")
    }
  }
  if (!is.null(sets)) {
    if (any(!unlist(sapply(sets, function(x) {
      x %in% colnames(W)
    })))) {
  ...
  if (is.list(validate)) {
    if (max(unlist(validate)) > n) {
      stop("Validation sets contains integer values larger than n = number of observations")
    }
  }
  if (!is.null(weights)) {
    if (!length(weights) == nrow(W)) {
      stop("nrow(W) not equal to length(weights)")
    }
  }

  # Compute G matrices
  if (is.null(sets) & is.null(G)) {
    G[[1]] <- (W %*% t(W)) / ncol(W)
  }
  nk <- length(G)
  nsets <- NULL
  if (!is.null(sets)) {
    ...
    for (i in 1:nsets) {
      G[[i + nk]] <- (W[, sets[[i]]] %*% t(W[, sets[[i]]])) / m[i]
    }
  ...
  if (!is.null(weights)) {
    vfm <- paste("~", paste(paste("G[[", 1:(nsets + 1), "]][t, t]", sep = ""), collapse = "+"))
  }
  vfm <- as.formula(vfm)
  ...
  for (i in 1:nvsets) {
    ...
    t <- (1:n)[-v]
    fit <- regress(fm, vfm,
      identity = identity, verbose = 1, pos = rep(TRUE, nsets + 1),
    ...
    f <- fv <- NULL
    for (j in 1:nsets) {
      f <- cbind(f, (G[[j]][t, t] * fit$sigma[j]) %*% fit$W %*% (y[t] - fit$fitted))
    }
    for (j in 1:nsets) {
      fv <- cbind(fv, (G[[j]][v, t] * fit$sigma[j]) %*% fit$W %*% (y[t] - fit$fitted))
    }
    if (nsets == 1 & nk == 0) {
      Vf <- NULL
      P <- fit$W %*% fit$Q
  ...
  return(list(
    f = f, fv = fv, Vf = Vf, s = s, vs = vs, sigmas = sigmas, pa = pa, mspe = mspe, ypred = ypred,
