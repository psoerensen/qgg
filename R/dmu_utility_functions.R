...
remlDMU <- function(fm = NULL, vfm = NULL, GRM = NULL, restrict = NULL, data = NULL, validate = NULL, bin = NULL) {
  ...
  model <- lapply(fm, function(x) {
    extractModel(fm = x, data = data)
  })
...
extractModel <- function(fm = NULL, data = NULL) {
  ...
  cvvar <- sapply(vvar, function(x) {
    x[2]
  })
  vvar <- sapply(vvar, function(x) {
    x[1]
  })
  names(cvvar) <- vvar
...
modelDMU <- function(model = NULL, restrict = NULL) {
  fixed <- unique(unlist(lapply(model, function(x) {
    x$fixed
  })))
  random <- unique(unlist(lapply(model, function(x) {
    x$random
  })))
  regression <- unique(unlist(lapply(model, function(x) {
    x$regression
  })))
  response <- unique(unlist(lapply(model, function(x) {
    x$response
  })))
  covmat <- NULL
...
writeDMU <- function(model = NULL, data = NULL, GRM = NULL, tol = 0.001) {
  ...
  for (i in 1:model$nt) {
    write(c(model[[i]]$n$random, model$data$random[model[[i]]$random]),
  ...
  for (i in 1:model$nt) {
    write(c(model[[i]]$n$regression, model$data$reals[model[[i]]$regression]),
      file = dir.file, append = TRUE,
  ...
  if (length(model$data$covmat) > 0) {
    for (i in 1:length(model$data$covmat)) {
      ...
      write(c("$VAR_STR", model$data$random[vvarname], "COR", "ASCII", vvarfile),
        file = dir.file, append = TRUE,
        ncolumns = 5, sep = " "
      )
    }
  ...
  write.table(format(data[, model$data$variables], scientific = FALSE),
    file = data.file,
...
rename.and.clean.windows <- function(jobname = NULL) {
  ...
  "my.system" <- function(cmd) {
    return(system(paste(Sys.getenv("COMSPEC"), "/c", cmd), show.output.on.console = FALSE))
  }
  ll.name <- c(
    "SOL", "PAROUT", "PAROUT_STD", "PEDDATA", "RESIDUAL",
    "LLIK", "SOL_STD"
  )
  ll <- ll.name[ll.name %in% ll.ff]
  for (kk in ll) {
    my.system(paste("move ", kk, " ", jobname, ".", kk, sep = ""))
  }
  junk.files <- c(
    "DMU1.log", "DMUAI.log", paste("COR", 1:20, sep = ""), "CODE_TABLE", "DMU1.dir", "DMUAI.dir", "DMU_LOG",
  ...
  if (length(del.files) > 0) {
    for (kk in 1:length(del.files)) {
      my.system(paste("del ", del.files[kk], sep = ""))
    }
  }
}
...
rename.and.clean <- function(jobname = NULL) {
  ...
  ll <- ll.name[ll.name %in% ll.ff]
  for (kk in ll) {
    system(paste("mv ", kk, " ", jobname, ".", kk, sep = ""))
  }
  junk.files <- c(
    "DMU1.log", "DMUAI.log", paste("COR", 1:20, sep = ""), "CODE_TABLE", "DMU1.dir", "DMUAI.dir", "DMU_LOG",
    "DUMMY", "FSPAKWK", "Latest_parm", "LEVAL", "MODINF",
  ...
  if (length(del.files)) {
    for (kk in 1:length(del.files)) {
      system(paste("rm ", del.files[kk], sep = ""))
    }
  }
}
...
writeGDMU <- function(G = NULL, filename = NULL, clear = TRUE, ldet = NULL) {
  ...
  nr <- nrow(G)
  if (!is.null(ldet)) {
    write.table(t(c(0, 0, ldet)), filename,
